...
| `MoviePilot-Plugins` | 发版、发布插件、版本升级 | `moviepilot-plugin-release` |
| `MoviePilot-Plugins` | 非发版插件 PR、CI 门禁、文档或普通维护 | `moviepilot-plugin-development`，完成后按目标再选择交付 |
...
- 用户只说“PR”：先检查当前仓库；主程序前后端走 `moviepilot-upstream-pr`。个人插件仓若是
  版本升级或发布准备，走 `moviepilot-plugin-release`；非发版插件 PR 先走
  `moviepilot-plugin-development` 核对范围与分支命名。官方插件仓走
