...
公共产品仓规则保留共同工程合同，提交准备与 checklist 面向 contributor 默认流程。工作区指令
承载已确认的维护者语境、授权复用和本地环境映射；两个 skill 据此选择适用验证、保存本地 anchor
及处置交付信号。维护者调整需有证据和明确安排，不改变架构、兼容、正确性或验证报告的真实性。
上游失败处置与后端协作规则共同采用 documented maintainer decision，可复用同一范围的预授权。
